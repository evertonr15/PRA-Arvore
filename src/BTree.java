public class BTree<Key extends Comparable<Key>, Value> {
	...
	private String toString(Node h, int ht, String indent) {
		...
		if (ht == 0) {
			for (int j = 0; j < h.m; j++) {
				valores.append(indent + children[j].key + " " + ((Pedido)children[j].val).toString() + "\n");
			}
		} else {
			for (int j = 0; j < h.m; j++) {
				if (j > 0)
					valores.append(indent + "(" + children[j].key + ")\n");
				valores.append(toString(children[j].next, ht - 1, indent + "     "));
			}
		}
